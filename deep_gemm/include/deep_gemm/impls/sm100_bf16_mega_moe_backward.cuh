...
template <
    ...
    uint32_t kNumWarps = kNumThreads / 32,
    uint32_t kNumComputeWarps = kNumWarps-1,
    uint32_t kNumComputeThreads = kNumThreads-32,
    uint32_t kGran = 8
>
...
sm100_bf16_mega_moe_backward_impl(
    ...
    DG_STATIC_ASSERT(kNumWarps >= 2, "Need at least one loader warp and one compute warp");
    ...
    comm::nvlink_barrier<kNumRanks, kNumSMs, kNumThreads, 0, 101>(
        workspace, sym_buffer, sm_idx, tid, [&]() { __syncthreads(); }, false, true);
    ...
    for (uint32_t idx = global_tid; idx < num_tokens*kNumTopk; idx += kNumGlobalThreads) {
        ...
        const auto slot = static_cast<uint32_t>(
            ptx::atomic_add(workspace.get_expert_send_count_ptr(expert_idx), static_cast<uint64_t>(1)));
    ...
    for (uint32_t i = global_tid; i < kNumExperts; i += kNumGlobalThreads) {
        ...
        *sym_buffer.map(workspace.get_expert_recv_count_ptr(sym_buffer.rank_idx, dst_local_expert), dst_rank) = count;
        ptx::atomic_add_sys(
            sym_buffer.map(workspace.get_expert_recv_count_sum_ptr(dst_local_expert), dst_rank),
    ...
    struct smem_shared final {
        alignas(1024) nv_bfloat16 x[2][BLOCK_M][kHidden];
        alignas(1024) nv_bfloat16 dy[2][BLOCK_M][kHidden];
        ...
        BlockDesc bd[2];
        bool bd_valid[2];
        Barrier stage_full[2];
        Barrier stage_empty[2];
    };
    auto& smem = *reinterpret_cast<smem_shared*>(smem_raw);
    if (tid == 0) {
        smem.stage_full[0].init(32);
        smem.stage_full[1].init(32);
        smem.stage_empty[0].init(1);
        smem.stage_empty[1].init(1);
    ...
    constexpr uint32_t kComputeSyncBarrierIdx = 1;
    const auto compute_block = [&](const BlockDesc& bd, const uint32_t& stage, const uint32_t& compute_tid, const uint32_t& compute_warp) {
        const bool is_shared = bd.local_expert >= kNumExpertsPerRank;
        const uint32_t valid_m = bd.valid_m;
        for (uint32_t idx = compute_tid; idx < valid_m*kHidden; idx += kNumComputeThreads)
            smem.dx_local[idx / kHidden][idx % kHidden] = 0.0f;
        ptx::sync_unaligned(kNumComputeThreads, kComputeSyncBarrierIdx);
        const uint32_t num_passes = is_shared ? kNumSharedExperts : 1;
        for (uint32_t pass = 0; pass < num_passes; ++pass) {
            const auto dw1 = (is_shared ? shared_dw1_weights : dw1_weights) + static_cast<uint64_t>(is_shared ? pass : bd.local_expert)*(2ull*kIntermediateHidden)*kHidden;
            const uint32_t w2_stride = is_shared ? kIntermediateHidden*kNumSharedExperts : kIntermediateHidden;
            ...
            for (uint32_t row = compute_warp; row < valid_m; row += kNumComputeWarps) {
                const auto* xrow = smem.x[stage][row];
            ...
            ptx::sync_unaligned(kNumComputeThreads, kComputeSyncBarrierIdx);
            for (uint32_t idx = compute_tid; idx < valid_m*kIntermediateHidden; idx += kNumComputeThreads) {
                const uint32_t row = idx / kIntermediateHidden, j = idx % kIntermediateHidden;
                const auto p = pg(j);
            ...
            for (uint32_t row = compute_warp; row < valid_m; row += kNumComputeWarps) {
                for (uint32_t j = lane; j < kIntermediateHidden; j += 32) {
                    ...
                    for (uint32_t n = 0; n < kHidden; ++n)
                        acc = fmaf(__bfloat162float(smem.dy[stage][row][n]),
            ...
            for (uint32_t row = compute_warp; row < valid_m; row += kNumComputeWarps) {
                ...
                if (lane == 0 && !is_shared) {
                    auto* remote_dw = sym_buffer.map(
                        bw.dtopk_weight_slot_buffer.get_rank_buffer(smem.src_topk[stage][row])
                            .get_data_buffer(smem.src_token[stage][row]).template get_base_ptr<float>(),
                        smem.src_rank[stage][row]);
                    *remote_dw = partial;
                }
                const auto w = smem.route_weight[stage][row];
                for (uint32_t j = lane; j < kIntermediateHidden; j += 32)
                    smem.dh[row][j] *= w;
            }
            ptx::sync_unaligned(kNumComputeThreads, kComputeSyncBarrierIdx);
            for (uint64_t idx = compute_tid; idx < static_cast<uint64_t>(kHidden)*kIntermediateHidden; idx += kNumComputeThreads) {
                const uint32_t n = idx / kIntermediateHidden, j = idx % kIntermediateHidden;
                float acc = 0.0f;
                for (uint32_t row = 0; row < valid_m; ++row) {
                    const float dO = smem.route_weight[stage][row]*__bfloat162float(smem.dy[stage][row][n]);
            ...
            for (uint32_t idx = compute_tid; idx < valid_m*kIntermediateHidden; idx += kNumComputeThreads) {
                ...
                const float sig = sigmoid<kFastMath>(gate);
                const float silu = gate*sig;
                const float dsilu = sig*(1.0f + gate*(1.0f-sig));
                const float dh = smem.dh[row][j];
                smem.dz[row][p] = gate_active ? dh*up*dsilu : 0.0f;
                smem.dz[row][p + kGran] = up_active ? dh*silu : 0.0f;
            }
            ptx::sync_unaligned(kNumComputeThreads, kComputeSyncBarrierIdx);
            for (uint64_t idx = compute_tid; idx < static_cast<uint64_t>(2*kIntermediateHidden)*kHidden; idx += kNumComputeThreads) {
                const uint32_t n = idx / kHidden, k = idx % kHidden;
                float acc = 0.0f;
                for (uint32_t row = 0; row < valid_m; ++row)
                    acc = fmaf(smem.dz[row][n], __bfloat162float(smem.x[stage][row][k]), acc);
        ...
        for (uint32_t row = compute_warp; row < valid_m; row += kNumComputeWarps) {
            auto* remote_dx = sym_buffer.map(bw.dx_slot_buffer.get_rank_buffer(smem.src_topk[stage][row]).get_data_buffer(smem.src_token[stage][row]).template get_base_ptr<nv_bfloat16>(),
                smem.src_rank[stage][row]);
            for (uint32_t k = lane; k < kHidden; k += 32)
    ...
    if (warp == 0) {
        ...
        for (;;) {
            ...
            if (iter >= 2)
                ptx::mbarrier_wait_and_flip_phase(&smem.stage_empty[stage], empty_phase[stage]);
            const bool done = block_idx >= *bw.num_blocks;
            if (!done) {
                const auto cur = bw.block_desc[block_idx];
                if (lane == 0) {
                    smem.bd[stage] = cur;
                    smem.bd_valid[stage] = true;
                }
                ...
                if (lane < cur.valid_m) {
                    ...
                    smem.src_rank[stage][lane] = src_rank;
                    smem.src_token[stage][lane] = src_token;
                    smem.src_topk[stage][lane] = src_topk;
                    smem.route_weight[stage][lane] = is_shared ? 1.0f : *sym_buffer.map(
                    ...
                    ptx::tma_load_1d(smem.x[stage][lane], remote_x, &smem.stage_full[stage], kHidden*2);
                    ptx::tma_load_1d(smem.dy[stage][lane], remote_dy, &smem.stage_full[stage], kHidden*2);
                    ptx::mbarrier_arrive_and_set_tx(&smem.stage_full[stage], 2u*kHidden*2);
                } else {
                    ptx::mbarrier_arrive_and_set_tx(&smem.stage_full[stage], 0);
                }
            } else {
                if (lane == 0)
                    smem.bd_valid[stage] = false;
                ptx::mbarrier_arrive_and_set_tx(&smem.stage_full[stage], 0);
            }
            if (done)
                break;
        ...
        const uint32_t compute_tid = tid-32;
        const uint32_t compute_warp = warp-1;
        uint32_t full_phase[2] = {0, 0};
        uint32_t stage = 0;
        for (;;) {
            ptx::mbarrier_wait_and_flip_phase(&smem.stage_full[stage], full_phase[stage]);
            if (!smem.bd_valid[stage]) break;
            const auto bd = smem.bd[stage];
            compute_block(bd, stage, compute_tid, compute_warp);
            ptx::sync_unaligned(kNumComputeThreads, kComputeSyncBarrierIdx);
            if (compute_tid == 0)
                ptx::mbarrier_arrive(&smem.stage_empty[stage]);
            stage ^= 1;
        }
    }
    __threadfence_system();
    comm::nvlink_barrier<kNumRanks, kNumSMs, kNumThreads, 0, 199>(workspace, sym_buffer, sm_idx, tid, [&]() { __syncthreads(); });
