...
static void sm90_bf16_mega_moe(
    ...
    const auto config = get_sm90_mega_moe_config(
        ...
        num_ring_tokens, MmaKind::BF16);

    // Make tensormap
    // NOTES: the token blocks are loaded as a whole (multicast), and the L1 output is stored per math warpgroup
    // as `[store_block_m, wg_block_n / 2]` BF16 tiles (64B or 128B swizzling)
    const int wg_block_n = is_sm90_mega_moe_ping_pong(config.block_m) ? config.block_n : config.block_n / 2;
    const int l1_output_block_n = wg_block_n / 2;
    const int l1_output_swizzle_mode = l1_output_block_n * static_cast<int>(sizeof(nv_bfloat16));
    ...
    const auto tensor_map_l1_output = make_tma_2d_desc(l2_acts,
                                                       ...
                                                       l1_output_block_n, config.store_block_m,
    ...
    const auto tensor_map_shared_l1_output = num_shared_experts > 0 ? make_tma_2d_desc(
        ...
        l1_output_swizzle_mode) : tensor_map_l1_output;
    ...
    // Compile
    const auto kernel = jit->compile("sm90_bf16_mega_moe", std::format(R"(
#include <deep_gemm/impls/sm90_bf16_mega_moe.cuh>

using namespace deep_gemm;

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&sm90_bf16_mega_moe_impl<
        {},
        {}, {},
        {}, {},
        {},
